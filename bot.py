...
def graph(bot, update, user_data):
     ...
     user_data['distance'] = (float(distance))/1000
...
def distribute(bot, update, user_data):
    ...
    if g == 0:
        ...
        bikes = int(bikes)
        docks = int(docks)
        d = user_data['distance']
        flow, max = data.distribute(d, bikes, docks)
        if flow == -1:
            errorhandler(6, bot, update)
            return
        bot.send_message(chat_id=update.message.chat_id, text = flow)
        bot.send_message(chat_id=update.message.chat_id, text = ("The maximum edge cost is", max[1], "to", max[2], "with", max[0]))
